...
- `merge-precheck-ordering` - settled: the warn half already fires (source-path WARNING + sidecar, `apex-merge/SKILL.md:49-52`); defer-until-discovery + harness-stash contradict the intentional unconditional auto-commit (`SKILL.md:39,82`). Sibling to `apex-merge-precheck-scope`.
...
- `review-doc-stale-inline` - inline review doc-stale fixes contradict the reviewer "Reports only - never edits" invariant (`agents/reviewer.md:72`); the clean invariant outweighs one saved round-trip. Explicit user wontfix.
...
- `doc-consistency-carveout-step11-suppress` - suppression conflicts with the carve-out's planned-scope keying (step-11 docs land AFTER the gate, `10-verify.md:26`); the code-only review hop is ~200-token noise. Explicit user wontfix.
- `review-rereview-skip-known-rejected` - an attempt-2 re-review skip needs new rejected-false-positive tracking state (`10-verify.md:70`); the gate-erosion risk outweighs one already-bounded reviewer call (cap 2 reviews + 1 fix). Explicit user wontfix.
- `reviewer-premise-precheck` - a pre-fix domain-contradiction gate can fail-closed against valid findings; the false-positive is ALREADY rejected at fix-dispatch (`08-execute.md` reconciliation). Sibling to `review-rereview-skip-known-rejected`. Explicit user wontfix.
- `lessons-step37-count-gate` - count-gating the Step 3.7 non-obvious filter skips mandatory leanness enforcement (`apex-lessons/triage.md` Step 3.7); drop=0 once does not predict the next, and the Step 0b simplified-mode gate already covers tiny sets. Explicit user wontfix.
- `pnpm-add-worktree-relink` - a pnpm virtual-node_modules relink rule would over-specify the generic executor contract (`agents/executor.md:36`); the existing "run the commands your change produces" rule already covers it. Niche datapoint. Explicit user wontfix.
- `decouple-complexity-hint` - a `decouple` `complexity_hint` variant conflates the magnitude axis (`04-hypothesis.md:5`) with a task shape, muddying the enum; the dead-code concern is already expressible via `alternatives` + `goals`. Explicit user wontfix.
- `merge-single-branch-discovery-overhead` - the unconditional `<run>-discovery.json` write is the complete-artifact-set-for-audit decision (`apex-merge/SKILL.md:85`); inlining for <=1 branch trades the audit trail for one round-trip. Sibling to `merge-sidecar-omit`. Explicit user wontfix.
...
- `gs-hook-split` - lowering the scout B0.7 `allowed_files` floor (`08-execute.md:35`) spawns scouts on tiny goals to catch a ~10-call overrun below both single-dispatch reaction thresholds. Bad cost/benefit. Explicit user wontfix.
...
Chronic-backlog triage (user-ratified wontfix; same pre-cluster drop + report contract):

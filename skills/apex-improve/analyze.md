...
- `apex-scope-overinclusion-docs` - doc pre-load cost already mitigated: `08-execute.md` E3.1/E3.6 surface goal-unmatched docs on-demand (never pre-loaded); the `doc_surface` fold (`discoverer.md:109`) is intentional.
- `review-doc-stale-inline` - inline review doc-stale fixes contradict the reviewer "Reports only - never edits" invariant (`agents/reviewer.md:72`); the clean invariant outweighs one saved round-trip.- `redundant-reread` - step-8 dispatch re-reading discovery-skip-inlined seeder files is already mitigated by `08-execute.md` E3.2 pre-read + E3.6 helper_snapshot inlining; low residual token-reduction, repeatedly bloat-deferred onto the hot-path file.- `step8-export-kind-verify` - single-session evidence never reinforced across 7 carries; one-off, not a pattern.- `admin-apex-inventory-post-skip` - token micro-optimization on an out-of-band skill; fast-path complexity outweighs the savings.- `review-gate-widen-fix-sessions` - widening raises review-dispatch cost on every fix session for 3-session anecdotal risk; the doc-consistency carve-out + complexity triggers (`10-verify.md`) already cover the surface.- `doc-consistency-carveout-step11-suppress` - suppression conflicts with the carve-out's planned-scope keying (step-11 docs land AFTER the gate, `10-verify.md:26`); the code-only review hop is ~200-token noise.- `review-rereview-skip-known-rejected` - an attempt-2 re-review skip needs new rejected-false-positive tracking state (`10-verify.md:70`); the gate-erosion risk outweighs one already-bounded reviewer call (cap 2 reviews + 1 fix).- `reviewer-premise-precheck` - a pre-fix domain-contradiction gate can fail-closed against valid findings; the false-positive is ALREADY rejected at fix-dispatch (`08-execute.md` reconciliation). Sibling to `review-rereview-skip-known-rejected`.- `lessons-step37-count-gate` - count-gating the Step 3.7 non-obvious filter skips mandatory leanness enforcement (`apex-lessons/triage.md`); drop=0 once doesn't predict the next; the Step 0b simplified-mode gate already covers tiny sets.- `pnpm-add-worktree-relink` - a pnpm virtual-node_modules relink rule over-specifies the generic executor contract (`agents/executor.md:36`); the existing "run the commands your change produces" rule covers it. Niche.- `decouple-complexity-hint` - a `decouple` `complexity_hint` variant conflates the magnitude axis (`04-hypothesis.md:5`) with a task shape, muddying the enum; the dead-code concern is already expressible via `alternatives` + `goals`.- `merge-single-branch-discovery-overhead` - the unconditional `<run>-discovery.json` write is the complete-artifact-set-for-audit decision (`apex-merge/SKILL.md:85`); inlining for <=1 branch trades the audit trail for one round-trip. Sibling to `merge-sidecar-omit`.- `merge-sidecar-omit` - conditionally omitting `<run>-precheck-auto-committed.txt` weakens the audit half of the settled merge-precheck-observability mitigation (`apex-merge/SKILL.md:49`) for a low token saving.- `gs-hook-split` - lowering the scout B0.7 `allowed_files` floor (`08-execute.md:35`) spawns scouts on tiny goals to catch a ~10-call overrun below both single-dispatch reaction thresholds. Bad cost/benefit.- `budget-guard-warn-skip` - the admin-apex task-3 `approaching-budget` WARN is intentional standing leanness pressure routed to `defer` (`audit.md:42`); skipping it deletes the signal, not the work.- `dcp-delegate-discipline` - reflector self-concluded "no structural change warranted"; the delegate-the-investigation idiom already exists (`apex-core.md` context-isolation), so a vague "strengthen" only bloats a central spec.- `admin-mirror-followup-seed` - the task-9 stage / task-10 mirror split is intentional (`admin-apex/SKILL.md:42`); seeding followups into task-9 conflates the two phases.- `economy-review-multifile` - widening economy-tier review triggers raises cost on the cheap path, eroding the tier's purpose; sibling to the settled `review-gate-widen-fix-sessions`.
...
- `audit-only-consolidated-sentinel` / `scout-predecompose-filehealth` - additive complexity on hot-path `08-execute.md` (dispatch-summary batching / file-health scout predictor) from single datapoints; Principle-3 accretion.
- `lessons-grep-keyword-flood-truncation` - mitigated by the screener direct-read fallback; the 240-line cap was deliberately tuned.
...
- `review-gate-partial-fix-escalation` - no concrete fix shape (undefined escalation, project-specific); sibling to `review-gate-widen-fix-sessions`.
- `planning-backend-feasibility` - single project-specific datapoint; a general missing-endpoint clause over-specifies the dense `04-hypothesis.md`.
...
- `discovery-feature-named-sibling` - single datapoint; feature-named-export adjacency widening risks the scope-noise `discovery-expand.sh` fights. Kept sibling `discovery-scope-claudemd` recurs.

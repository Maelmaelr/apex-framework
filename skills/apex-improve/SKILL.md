...
**Near-cap tightening (proactive leanness)**: when an edit touches a skills/agents .md already above 85% of its content-budget tier (`skills/apex/scripts/content-budget.json`; flagged by the `approaching-budget` audit / polish detector), tighten existing prose in the SAME pass for a net-negative word delta - do not merely avoid growth. Caveman house style: drop articles / copulas / connective filler, collapse redundant restatement; keep every numeric threshold, MUST / SHOULD / conditional, and incident citation (`reflector <hex>`). This is the standing-pressure complement to the hard file-health hook (which gates at 100% of tier).

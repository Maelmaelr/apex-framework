...
After each step below, append one line to `.claude-tmp/lessons-extract-active/{RUN}-summary.md` in the format `step-{N}: <outcome>` (e.g., `step-1: tmp=12 lines master=523 lines`, `step-2: 8 added / 1 skipped-duplicate`, `step-5: 8 lessons extracted`). Captures friction the linear text steps don't (early-exit reasons, pre-check size jump, write volume); read by Step 6's reflector. **Counts-only contract**: each trace line is one outcome aggregate - never inline lesson titles, lesson bodies, full lesson lists, or narrative parentheticals like `(1 already verified)` into `{RUN}-summary.md`. When any lessons are skipped before write (already present in lessons.md by content hash, malformed, etc.) the step-2 aggregate MUST encode the skip count explicitly as `N skipped-duplicate` rather than parenthetical prose - the friction signal is the count, not its prose explanation. The trace exists for friction signals, not for re-presenting the lesson surface that lessons.md / lessons-tmp.md already hold.

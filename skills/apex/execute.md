...
- **Dispatch self-report log (E1)**: append each return to `.claude-tmp/apex-active/{session}-traces/execute/dispatch-summary.json` (JSON array; append-and-rewrite). The executor self-report is gameable, so the orchestrator MUST also attach the executor Agent return's ACTUAL harness usage - `tool_uses`, `total_tokens`, `duration_ms` from the Agent tool result - and a git-reconciled `files_touched` (`git diff --name-only {baseline.head_sha}` intersected with the task scope) beside the self-reported `tool_calls_made` / `files_touched`. Step 13 reflector keys its oversized-dispatch trip on this actual telemetry, never the self-report alone (reflector fa269898 user-flagged x2: self-report 71 `tool_calls_made` undercounted the real 134 `tool_uses` / 256k `total_tokens` ~2x; 145df100 / da97c3c1: self-reported `files_touched` under-reported the git diff).

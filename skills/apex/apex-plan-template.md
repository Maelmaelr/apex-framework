...
**Chain-trace for propagation ACs:** When an AC asserts a value "propagates into a response" or is "returned by endpoint", enumerate every hop in the delivery chain (e.g., controller -> service -> async callback -> DB row -> poll endpoint). ACs that leave intermediate hops unspecified produce implementations that discard the signal at the first gap, requiring a delta roundtrip.

**Type location lookup:** When an AC names a type or interface to modify ("add field to interface X"), grep the type name at plan-write time (`grep -r "interface X\|type X"`) to confirm the actual file path -- guessing from semantic domain (e.g., placing a model type in a sibling type file) is a recurring plan error that causes teammate scope violations.

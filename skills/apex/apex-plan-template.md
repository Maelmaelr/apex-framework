...
- Complexity and file health: Include all CLAUDE.md Code Quality constraints (cognitive complexity max 15, file health >400/500 line gates, barrel exports) as acceptance criteria in goals that modify at-risk files. Check file line counts from scan/scouts to identify at-risk files. When a goal extends an existing method with conditional branches, include an AC to pre-check the method's current cognitive complexity and extract/refactor first if near the limit (>10). For goals that add new code to files already above the 500L hard gate (per scan file health notes), the plan MUST include a pre-extraction teammate task (or sequenced sub-task) scheduled before the content-adding task runs. The pre-extraction task separates a logically cohesive section -- not the newly added content -- to bring the file under the gate. Do NOT merge pre-extraction and content addition into one task: splitting scope from addition keeps teammate focus and avoids mid-implementation gate blockers.
...
- Destructured-prop edit sites: When a goal adds props to components with destructured function params, AC must list all 3 edit sites: (1) type/interface definition, (2) destructuring parameter list in function signature, (3) usage/JSX. Missing the destructuring list is a common build failure -- the type compiles but the value is undefined at runtime.
...
- Type boundary friction: When a plan routes data across layer boundaries (validator -> controller -> service), flag transformation points where the validator's output type differs from the domain type (e.g., comma-separated string from form input vs. array in domain model -- the controller must parse). When builder/factory functions return discriminated union types consumed by downstream code, flag access sites that need explicit type narrowing or casting (e.g., accessing a platform-specific field on a union of TikTok | YouTube metadata requires narrowing first). When a goal adds service helpers that return objects with optional fields (e.g., `reason?: string` vs `string | null`), include an AC note that call sites must narrow the optional field before use -- mismatched optionality (`reason?: string` treated as `string`) is a common source of type errors during verification that are cheaper to anticipate in the plan. Include these transformation/narrowing points as notes in the owning teammate's acceptance criteria so they handle them during implementation rather than surfacing as build errors during verification.
...
- Delegate signature verification: When a goal involves converting bare re-exports to wrapper functions (e.g., adding rate limiting or middleware to route handlers), acceptance criteria must include verifying the delegate's TypeScript signature before wrapping. Framework handlers (NextAuth, platform shims) may accept only `(request: NextRequest)` -- passing a second `context` argument causes type errors or silent behavior mismatch. Include: "Verify delegate function signature and match wrapper call arguments accordingly."
...
- Item counts: When the task involves consolidation, migration, or merging across files, include concrete item counts from scout findings in each goal. Distinguish countable items (violations, files, BP-IDs) from volume metrics (line reductions, file size changes). Countable items use actual enumerated counts from scouts. Volume metrics use directional language ("significant reduction", "substantial cleanup").
...
- Inline numeric test-case values: When acceptance criteria include worked examples with numeric inputs and expected outputs (e.g., "at X credits with Y% discount, result = Z"), the AC must direct the teammate to read the source (pricing table, tier constants, config) and derive values rather than trusting the plan's embedded numbers. Plans hardcode example tier percentages (5% vs actual 10%) that drift from source between scan and implementation. Phrase the AC as "verify against {source-file}" not as a fixed numeric assertion.
...
{List ALL runtime activation commands required for this implementation to be complete. The verifier (apex-verify.md Step 4) executes these commands after build passes. Include migrations, seeders, code generation, external service provisioning (Stripe CLI, cloud resource creation), and dependency installs. If none needed, write "None."}

- {command 1, e.g., `cd apps/api && node ace migration:run`}
- {command 2, e.g., `stripe products create --name="Pro Plan" --default-price-data.unit-amount=2999 --default-price-data.currency=usd --default-price-data.recurring.interval=month`}
...
**Type propagation ownership:** When goals add members to cross-component types (shared enums, union types, discriminated unions), grep consumer files (`grep -r "TypeName" --include="*.ts" --include="*.tsx"`) to discover the full propagation graph -- switch/case handlers, type guards, display components, barrel re-exports, i18n keys. Additionally grep for `Record<TypeName>` literals across source AND test directories (`grep -rn 'Record<TypeName' apps packages --include='*.ts' --include='*.tsx'`) -- each Record literal is a mandatory ownership inclusion because TS exhaustiveness failures in test fixtures break build the same as source. Include all discovered consumers in ownership. Plans adding union members routinely underestimate web-side scope by 3-5 files. Additional rule for **union narrowing** (removing or restricting a literal member, e.g., `'external' | 'flowctory'` -> `'flowctory'`): also grep the bare literal value -- `grep -rn "'{removed-literal}'" apps packages --include='*.ts' --include='*.tsx'`. Construction sites (builders, factories, sibling helper files with internal interfaces that mirror the type) write the literal string directly and do not surface via TypeName grep. Include all matching files in the narrowing teammate's ownership.
...
**Shared component prop ownership:** When a goal adds new props to a shared UI component, assign that component file to the teammate driving the prop change. Otherwise the teammate modifies outside its scope with no oversight. Same rule applies in reverse: when a goal removes or narrows a shared component's props, assign the component file to the teammate driving the removal -- other consumers may need prop-change propagation.

**Symbol removal ownership:** When a goal removes an exported symbol (component, type, function) from a subfolder, include ALL ancestor-folder barrel files that re-export it in the owning teammate's ownership. Grep: `grep -rn "from ['\"]\./{subfolder-name}" <package>/**/index.ts` plus named re-exports matching the symbol name. Parent-folder `export * from './sub'` and named re-exports escape the subfolder-barrel scope and surface as stale-export build/lint warnings at verification.
...
**Shared-helper determinism:** When two or more teammates will consume the same helper function, the plan must commit to a single reuse approach in ACs -- "extract to a dedicated shared file" OR "import in place from its current location" -- and never offer both as alternatives. Phrasing an AC as "extract OR import in place" lets the dependent teammate pick, producing duplicate helpers or skipped extractions that surface only at lead verification. Additionally, if the referenced helper is currently private (not exported from its module), state the export action as an explicit AC line ("Export `helperName` from `{path}`") owned by the source-file teammate. Non-exported helpers are a routine mid-implementation blocker that is trivially resolvable in the plan.
...
Adapt to the actual endpoints and shapes. Before writing contract shapes, grep existing API controllers for the project's response envelope pattern (resource-named keys like `{ "generation": {...} }` vs generic wrappers like `{ "data": {...} }`). Match the existing convention -- mismatched envelopes cause cross-boundary fixes during verification. Include concrete field names, nesting structure, and types for both success and error responses. This prevents two classes of mismatch:
1. URL mismatches (one agent implements a route, another calls a different path)
2. Shape mismatches (flat vs nested response objects, divergent error field naming like `message` vs `error`)
...
**Enum granularity.** When contracts specify enum fields (status codes, error reasons, event types, failure categories), list only distinct semantic categories -- not every variant string that could ever appear. Over-specification forces teammates into artificial distinctions at implementation sites where logically-equivalent cases must collapse (e.g., "parse-error" and "group-data-missing" are indistinguishable at the point of parser failure). Enumerate what downstream consumers actually branch on; leave finer-grained distinctions to free-text detail fields. Rule of thumb: if two enum values would trigger the same recovery path, collapse them into one.
...
**Placeholder gate:** Before calling ExitPlanMode, grep the plan file for the literal strings `{session-id}`, `{task-type}`, `{prd-path}`, `{audit-path}`. If any found, fix before proceeding.

...
- Both read `git diff {diff_anchor}`. `learn` appends to `.claude-tmp/lessons-tmp.md` under `flock` (via `bash skills/apex/scripts/append-with-lock.sh`); the agent's own bar drops within-session trivia even when the gate opens. `learn` is project-specific (codebase patterns); the step-13 `reflector` is apex-specific (workflow / pipeline improvements).

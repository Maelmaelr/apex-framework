...
4. ANY of:
   ...
   - ANY `hypothesis.goals[]` matches `/\b(rewrite|migrate|redesign|refactor|new endpoint|new component|new feature|billing|pricing|cost|parity|invariant|across all|webcam|camera|microphone|geolocation|permission|sdk)\b/i` (the billing/pricing/cost/parity/invariant class fires review on semantically significant medium-complexity correctness/parity changes the raw file-count gate misses; the webcam/camera/microphone/geolocation/permission/sdk class is the security-surface trigger - device-permission + external-SDK goals warrant review regardless of complexity_hint), OR
...
**Doc-consistency carve-out** (overrides 3-4; conditions 1-2 still required): even when conditions 3 and 4 do not both hold, the gate fires when `allowed_files` (the planned scope, NOT the touched-at-gate-time set - step-11 docs land AFTER this gate, so a touched-set check under-counts doc-heavy sessions) contains >= 1 doc/spec path (`*.md` / `docs/**` / `CLAUDE.md` / `.claude/rules/**`) AND the touched INTERSECT `allowed_files` set contains >= 1 non-doc code path. This catches small (often 2-file) doc+code co-changes the >= 3 size gate would otherwise skip, so a change cannot silently leave an in-scope doc contradicting the new code. Standard tier only for v1 (economy / trivial unaffected - preserves the economy-skip invariant; economy coverage is a noted future extension).
...
1. **Evaluate gate**. If any condition fails, write `$APEX_ACTIVE/{session}-review-skipped.json` (absolute via the resolver above) with `{reason: "<one-line>"}` so `/apex-improve` can audit gate effectiveness, then return to caller without spawning.
...
2. **Spawn `agents/reviewer.md`** (Sonnet, foreground). Agent returns the JSON shape defined by `skills/apex/schemas/review-result.schema.json`; orchestrator producer-validates via `bash skills/apex/scripts/validate-json.sh review-result.schema.json <agent-output-path>`.
   ...
   **Read-only assertion (reviewer-mutation-guard)**: snapshot `git status --porcelain` immediately before this spawn and again after the agent returns - the reviewer's only repo write is the gitignored `.claude-tmp/` trace (porcelain excludes it), so the two snapshots MUST match. A non-empty delta means the reviewer edited tracked/untracked files against its read-only contract: do NOT accept the returned `action` (a reviewer can never `pass` on its own changes). AskUserQuestion (`keep-edits-and-proceed` | `revert-and-rereview` | `abort`; dismiss = `abort`) before branching at step 3 - `revert-and-rereview` discards the reviewer-touched paths and re-spawns attempt 1; `abort` runs `session-end-hook.sh {session}` inline. Mirrors the step-8 `git status --porcelain` post-dispatch reconciliation (`08-execute.md` E1).

3. **Branch on `action`**:
   - `pass` -> proceed to step 11 (no trace; silent green).
   - `fix-needed` -> dispatch `agents/executor.md` ONCE (Sonnet, cap 1, no retry). Spawn-prompt carries the findings list as the fix scope (no `goal` field; this is a fix-only invocation analogous to step 10's fix-loop). After the executor returns, spawn `agents/reviewer.md` again with `attempt=2` + `prior_findings_path` to confirm fixes landed. **Mechanical-fix inline allowance**: when every finding is a zero-ambiguity surgical edit (dead-code delete, void-pattern early-return, unused-import strip) AND finding count <= 2, the orchestrator MAY inline-Edit the fix instead of dispatching the executor; on inline-edit it MUST set `$APEX_ACTIVE/{session}-fix-attempts.json` (absolute via the resolver above) to the producer-validated object `{count: N, ...}` (recording the inline-fix `kind: "review-inline"` + `files: [<paths>]` as extra context fields - the schema is an object, not an append-array) BEFORE step 11 so the learn difficulty gate (`count >= 1`) observes the rollup and learn runs (mirrors the step-8 cross-scope-inline pattern).
...
4. **Attempt counter**: maintain `$APEX_ACTIVE/{session}-review-attempts.json` (absolute via the resolver above; cap 2 reviews + 1 fix = max 3 hops total). On `attempt=2` returning `fix-needed` again (the fix did not resolve), the orchestrator MUST escalate (no third attempt; force AskUserQuestion).

...
**Scope completeness.** Before spawning, verify the scope file includes companion `.spec.ts`/`.test.ts` files for each owned source file (Glob to discover them; add matches to the `files` array). Also spot-check 1-2 listed paths to confirm they resolve to actual files -- plan-sourced paths are sometimes wrong (wrong directory), blocking the first Edit. Fix mismatches before spawning.
...
Before any implementation: create ALL tasks via TaskCreate (Phase 2 step 4 in apex-teammate-workflow.md). This is a HARD GATE -- do not begin implementation until TaskCreate calls are complete and verified with TaskList.
For tasks where ACs reference "same logic as {other teammate's helper}" or "reuse {helper}": IMPORT that helper rather than re-implementing it. Two independent implementations of the same logic diverge on subtle details (count sources, edge handling) and require a delta roundtrip to reconcile -- reuse is always cheaper.
...
2. **Scope verification.** Follow subagent-delegation.md item 10 scope check procedure using `.claude-tmp/apex-baseline-{team-name}.txt` as the baseline file (per shared-guardrails #4). **Baseline self-heal.** If `.claude-tmp/apex-baseline-{team-name}.txt` is missing (e.g., unrelated stale-cleanup side effect mid-session), recreate it from current dirty state: `(git diff --name-only; git ls-files --others --exclude-standard) | sort > .claude-tmp/apex-baseline-{team-name}.txt`; print `BASELINE REBUILT: {team-name}`. Rebuilt baseline over-attributes current dirty files as pre-existing (false-negative risk, not false-positive) -- re-verify scope violations manually for this teammate. Compare session-only changes against the reporting teammate's owned files list. If any file outside their ownership was modified or created, report the scope violation and use AskUserQuestion: "Scope violation: {teammate} modified files outside ownership: {files}. Revert these files / Keep changes / Review diff first". Do NOT automatically revert or delete files (per shared-guardrails #5). Batch the git-status diff check and AC spot-check greps into a single parallel tool-call round (one Bash + one Grep batch) rather than sequential calls per file.
2a. **Dirty-state sanity check.** After scope verification, run `git status --short` and compare the file list against the teammate's reported modifications. If the working tree shows FEWER dirty files than the teammate reported (i.e., reported files are missing from git status), an external stash, reset, or parallel session side-effect may have silently wiped the work. Surface via AskUserQuestion: "Teammate {name} reported modifying {files} but git status shows those files are clean. External stash/reset suspected -- recover from stash / Proceed anyway / Investigate". Never silently accept a completion report whose working tree evidence is absent.
...
3. Review result against acceptance criteria. Before flagging a gap, check whether another teammate's already-reported changes achieved the goal implicitly (a broader change in one teammate can satisfy a sibling goal as a side effect). **Hedge language = cross-boundary failure.** If the completion report includes phrases like "worth a grep before merge", "confirm downstream handles X", or "may need updating" -- treat as a verification failure, not an advisory. The hedge signals the teammate lacked context to verify the cross-boundary contract. Either grep the referenced file yourself now, or SendMessage the owning teammate to fix it before accepting the report.
...
5a. **Pre-spawn deliverable verification.** Before spawning a dependent teammate whose acceptance criteria reference an upstream teammate's output, Read the upstream deliverable file(s) -- do not rely on the completion report alone. Verify contracts, threading, and exports are in place. Missing/incomplete: SendMessage the upstream teammate for a fix before dependent spawn. Cheaper than letting the dependent build on an incomplete foundation.
...
7. **Early shutdown.** If the teammate's work is verified (scope clean, acceptance criteria met) and no further fixes are expected from them, send shutdown_request immediately rather than waiting for all teammates. This reduces idle notification noise during the remainder of the session.
    ...
    **Pre-shutdown AC spot-check.** Before sending shutdown_request, source-spot-check each explicit path or call site referenced in this teammate's ACs against at least one file they modified. A build-clean completion report with unmet ACs produces a silent broken chain that only surfaces at lead verification or runtime. Do not trust the report alone -- verify one grep per AC against actual file content.

    **Out-of-scope callout gate.** If the teammate's completion report includes an "out of scope but required" callout (e.g., "X must be done but is outside my ownership"), treat it as a verification failure -- do NOT accept the completion and send shutdown_request. Instead, either (a) assign the missing work to the owning teammate via SendMessage, or (b) implement it directly as lead. An out-of-scope callout means the end-to-end chain is broken; accepting the report silently ships the gap.

    Before sending shutdown_request, if the reporting teammate's Changes summary includes delete/remove or rename verbs for exported symbols, grep unreported teammates' ownership files for the deleted/renamed symbol names. If matches found, either (a) SendMessage the owning teammate with the affected file list so they can fix the reference in-session, or (b) note for lead-implement follow-up per item 4. Only then send shutdown_request. Reactive handling at item 4 relies on teammates self-flagging cross-boundary references; the active pre-shutdown scan catches references in teammate work not yet reported.

    Additionally, if the reporting teammate owned test files AND other teammates are still implementing AND the session modifies shared modules consumed by tests (new exports on shared barrels/hooks, refactors of vi.mock-ed modules), defer shutdown until the lead has run build + tests once against the post-implementation codebase. Test-specific regressions (mock-factory shadowing, new-export pass-through, stale imports) typically surface only at verification and require the test-owner's context to fix efficiently.
...
4. **Cross-boundary resolution check.** For each cross-boundary issue flagged in Step 3: if test-affecting (stale mocks, broken imports, renamed services), fix before proceeding to verification -- scoped test runs derive from the modification list and will not catch unmodified-but-broken test files. If refactoring-only (duplication, extraction candidates), note as post-session follow-up. Do NOT defer test-affecting issues to verification. **Concurrent-session mock divergence.** If this session ran concurrent APEX sub-sessions or if any teammate modified a shared barrel/hook that test files `vi.mock`, grep for `vi.mock` targeting those shared modules across all test files in the modification list. A scoped test that passed in the teammate's session may fail at lead verification if a concurrent change wired a new export that the mock doesn't satisfy.

...
- `discovered_paths: [<paths>]` (optional) - repo-relative paths captured via bounded inline Glob / Grep / Read when the prompt is conceptual (no path tokens in `original_prompt`). Cap exploration at ~5 calls; persist only paths validated on disk (cap-5 is LLM-side discipline at the call site - artifact has no call count to enforce post-write; consumers `_enumerate.py` + `zero-layer-extract.sh` re-validate path existence and silently drop misses). Consumed by 6.a enumerate as seed_paths AND by 6.a zero-layer dispatch as autonomous fallback scope (no AskUserQuestion when populated). Skip discovery when the prompt names files - regex extraction covers them; one exception, when a named file is a routing / registry / index entry-point (e.g. `routes.ts`, `controllers.ts`, `index.ts`), Glob for same-prefix sibling files (`routes_*.ts`, `controllers_*.ts`, `index_*.ts`) since these patterns commonly split across multiple files.

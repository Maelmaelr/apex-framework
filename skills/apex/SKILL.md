...
14. **Cleanup session** - `bash skills/apex/scripts/cleanup-session.sh --session {session} --post-success`. Idempotent. `--post-success` bypasses the live-PID guard for trusted own-session callers. Does NOT clean `{session}-hypothesis.json` (consumed by step 15). **Post-apex commit audit**: also runs `git log {baseline_head_sha}..HEAD --pretty=format:'%h %an %s'` and surfaces commits within the session window NOT authored by the git-sync agent (subject does not start with `apex git-sync`) as a one-line warning fed to step 15 summary - flags post-apex commit creep unrelated to hypothesis (reflector 88ba4171: user-management/user-tracker commits added post-apex outside scope).

...
1. **Analyze** - inline. AskUserQuestion if ambiguous; abort = clean exit (no manifest yet, skip session-end-hook). Read `<project-root>/docs/project-context.md` (best-effort; absent = silent skip; cached for downstream; cap read at ~200 lines). When formulating the working interpretation, expect step 4 to emit a `goals[]` decomposition: single-task prompts -> one goal; multi-task / audit prompts ("make sure all X are properly Y") -> N enumerated goals. Internal bias only; no user-visible change at step 1.
   - **Early scope narrowing.** When the original prompt implies a multi-phase plan (>3 implied phases, OR >5 enumerated subtasks, OR explicit phase / step / roadmap language), AskUserQuestion BEFORE step 4 to narrow scope (`audit-only` | `phase-1-only` | `full-scope` | `let-me-respec`); do NOT defer to a step 8 re-conversation after goals are drafted (reflector 9dd0d177: "Step 8 scope re-conversation happened late; pre-run conversation could surface this earlier"). Cheaper to narrow once at step 1 than to discard a 6-goal hypothesis at step 8.
...
9. **Polish** - agent `agents/polish.md` (Sonnet). **Skipped on economy tier** (mirrors learn-skip rule; economy already implies tiny scope: `len(goals)==1 AND allowed_files<=5 AND no rewrite verbs`, so the surface for orphan imports / dead code is small and step 10 verify catches functional regressions). Spawn-prompt: `session`, `main_scope_path`, `baseline_head_sha`, `lessons_hits` (advisory; subagents do NOT inherit working memory). Agent computes touched-by-apex set (`(git diff --name-only {baseline.head_sha}; git ls-files --others --exclude-standard) | sort -u`) INTERSECTED with `allowed_files`; performs in-scope-only fixes (unused imports / dead code / leftover comments / naming). Hard cap = intersected set; scope-check hook is outer guard. Returns one-line summary; no-op exit if nothing actionable.

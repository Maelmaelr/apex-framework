...
- `discovered_paths: [<paths>]` (optional) - repo-relative paths captured via bounded inline Glob / Grep / Read when the prompt is conceptual (no path tokens in `original_prompt`). Cap exploration at ~5 calls; persist only paths validated on disk. Consumed by 6.a enumerate as seed_paths AND by 6.a zero-layer dispatch as autonomous fallback scope (no AskUserQuestion when populated). Skip discovery when the prompt names files - regex extraction covers them.

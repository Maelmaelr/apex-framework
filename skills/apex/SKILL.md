...
**Migration-log gate.** Before handing off, run `bash ~/.claude/skills/apex/scripts/check-migration-log.sh` from the worktree root. A non-zero exit means you committed a migration file whose apply command never reached the side-effects log - the merge carries the file but `/apex-merge` step 4.5 replays only the log, so the base DB stays un-migrated and drifts from the code. Fix it (append the verbatim apply command to `.claude-tmp/apex-active/<session>-side-effects.jsonl`, or dispatch an executor to apply+log it) and re-run until it passes. Do not hand off on a red check.

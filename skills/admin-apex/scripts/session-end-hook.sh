...
# each manifest whose cc_session_id matches the ending session.
#
# Manifests not produced by admin-apex / apex-improve (no cc_session_id field,
# or unparseable JSON) are skipped silently - the hook is forgiving so that a
# malformed sibling artifact never blocks cleanup of valid runs.
#
...
# No matching manifest -> non-/admin-apex CC session, nothing to clean.

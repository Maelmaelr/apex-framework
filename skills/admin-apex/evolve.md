...
| `negative-scope` | `edit` per flagged doc - delete the disclaimer section heading + its bullets, or rewrite the third-person `Does NOT ...` bullet as the positive rule it implies (keep operational negatives; drop pure non-responsibility prose). WARN-class like `hash-roster`; usually `defer` unless the user picks `apply`. `doc_only` per the standard rule. |
...
Use `Glob` and `Grep` (or `scripts/grep-apex-refs.sh`) BEFORE any `Edit` to confirm each ref site. NEVER blind-edit. Edits follow apex-core.md Conventions **Lean prose**: state rules positively, no `What X does NOT do` / `Out of scope` sections or incident-narrative justifications; cite a cluster slug, never a raw `reflector <hex>` hash.

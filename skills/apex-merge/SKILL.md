...
1. **Precheck** - inline. Refuse to run outside the main worktree:
   ...
   ```
   Main worktree may be dirty. `.apex-worktrees/` is untracked-by-design; filter that line before measuring. When `DIRTY_COUNT <= 10`, anything else is auto-committed inline ("just commit, don't ask", run 03d9a286); user runs `git restore` / `git stash` manually if needed. When `DIRTY_COUNT > 10`, orchestrator MUST AskUserQuestion BEFORE staging (`auto-commit-all | abort-to-review`; dismiss = abort-to-review) with the full `git status --porcelain` output (filtered as above) listed verbatim in the prompt so the operator sees the WIP set before it lands as a single opaque commit; on abort, exit 1 with explicit error (reflector 53bc6e73: 36 unrelated WIP files swept silently into a pre-merge commit). First unstage any `.apex-worktrees/*` mode-160000 gitlinks (residue from prior `git add .`; reflector ba0afe92 + 32455372). The follow-on `git add -A` MUST carry `':!.apex-worktrees'` so it does not re-stage what `git rm --cached` just unstaged:
   ```bash

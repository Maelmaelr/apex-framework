...
1. **Precheck** - inline. Refuse to run outside the main worktree:
   ...
   ```
   Main worktree may be dirty. `.apex-worktrees/` is untracked-by-design (its mode-160000 gitlinks must never be committed). Per explicit user request, project `.claude/` + `.claude-tmp/` ARE swept into the auto-commit and land on main (operator config + apex artifacts integrate with the merge); everything else is auto-committed unconditionally with NO dirty-count prompt ("just commit, don't ask" - user runs `git restore` / `git stash` manually if needed). When the dirty set contains real source paths (per `scripts/dirty-classify.sh` - everything outside harness state + the shared auto-generated/regen allowlist), the code below emits a one-line warning naming them before committing so mid-edit files stay visible; the verbatim `git status --porcelain` set lands in `${RUN}-precheck-auto-committed.txt` for audit. The auto-commit MUST NEVER carry `--no-verify` (CLAUDE.md non-negotiable; pre-commit hook failure -> AskUserQuestion `abort-merge | retry-after-fix`, never bypass). First unstage any `.apex-worktrees/*` gitlinks; then a `git ls-files -u` guard aborts on unmerged (UU) index entries left by an orphaned stash-pop, so conflict markers are never staged onto main (cluster: merge-precheck-unmerged-index). The follow-on `git add -A` MUST carry `':!.apex-worktrees'` so it does not re-stage what `git rm --cached` just unstaged:
   ```bash
   ...
   if [[ "$DIRTY_COUNT" -gt 0 ]]; then
     ...
     SRC_DIRTY=$(printf '%s\n' "$DIRTY_LIST" | bash skills/apex-merge/scripts/dirty-classify.sh --filter-src || true)
     if [[ -n "$SRC_DIRTY" ]]; then
       SRC_TOTAL=$(printf '%s\n' "$SRC_DIRTY" | grep -c . || true)
       printf 'apex-merge precheck WARNING: auto-committing %d source path(s) to main:\n%s\n' "$SRC_TOTAL" "$SRC_DIRTY" >&2
     fi
...
4. **Merge loop** - `bash skills/apex-merge/scripts/merge-loop.sh <run>`. Per branch with commits past base:
   ...
   - **Exit 2 (dirty-tree gate)**: files dirtied between Step 1 precheck and the loop abort with exit 2 (merge-loop.sh:75,84) BEFORE any branch is touched - distinct from exit 20 (conflict) / 21 (merge-refused). Recover by re-running Step 1's filtered auto-commit (unstage `.apex-worktrees` gitlinks -> `dirty-classify.sh` source-WARNING -> `git add -A -- ':!.apex-worktrees'` + commit, NEVER `--no-verify`), then re-invoke `merge-loop.sh <run>`.

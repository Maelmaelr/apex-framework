...
- Tail -> drop unless the file is named in `hypothesis.discovered_paths` or implicated by a goal noun beyond mere import / test usage (3 kept tail files were never edited - kept context, not kept work).
...
- Read-only doc/rules gate: when a kept file is a doc/rules path (`docs/**`, top-level `*.md`, `CLAUDE.md`, `.claude/rules/**`) that no `hypothesis.goals[]` / `discovered_paths` entry names as a write target, keep it but prefix its `screener_reason` with `READ-ONLY: `. The discoverer / steps/08-execute.md E3 preamble then surfaces these under `Shared read-only context (do not write)` so executors skip full re-reads instead of paying ~20k tokens/run re-reading scoped-in-but-unmodified docs (6 docs/rules files in scope, only 1 impacted; precautionary architecture-api.md / canvas-patterns.md both read-only).
- Read-only parity/verify gate: a kept *code* file that grep matched but that no `hypothesis.goals[]` names as a write target, whose role is parity audit / divergence check / verification only (an upstream fetcher, a pricing-lookup peer, a registration mirror, a read-only web-fetch target), gets the same treatment as the doc/rules gate above - keep it but prefix its `screener_reason` with `READ-ONLY: ` so the discoverer / steps/08-execute.md E3 preamble hands it to the executor as inline-read context rather than a full scope file (14 read-only web-parity files; 4 parity-audit files - both routed as full scope with zero edits).

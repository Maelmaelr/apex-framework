...
- `allowed_files` - the goal's resolved per-task scope subset (the ~3-8 files step 8.2 narrowed to, OR a larger high-cost set routed here before B1 per steps/08-execute.md B0.7). This is the HARD ceiling on `subtasks[].files`.
...
5. When the goal is one coherent change that cannot be split without creating a shared-contract race (a single function rewrite, one tightly-interlocked edit), return `indivisible:true` with `subtasks: []` - the orchestrator dispatches the goal whole as today. Default to `indivisible:true` on genuine uncertainty: a wrong split costs a wasted spawn + a re-merge, the deliverable is the DAG only when the independence is clear. **Bug-localization caveat**: when the goal is a bug FIX (not a feature build) and the candidate split is by gating / handler layer (per-node vs central catch-all, per-handler vs shared), do NOT split on a layer-hypothesis - the defect's true layer is unverified until an executor investigates, so a wrong layer-split dispatches an executor that finds no defect and forces a re-dispatch. Return `indivisible:true` (or `depends_on`-chain only when a confirmed fix in one layer feeds the next) so one executor localizes then fixes (cluster: scout-layer-mislabel).

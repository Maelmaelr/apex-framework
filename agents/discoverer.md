...
- `hypothesis` - JSON-serialized contents of `{session}-hypothesis.json` (`hypothesis`, `complexity_hint`, `discovered_paths`, `alternatives`).
...
Spawn-prompt: ranked top-K (default 30) + hypothesis (verbatim from this agent's spawn input). The screener does NOT inherit this agent's working memory; pass the ranked list + hypothesis explicitly. Output: `{kept: [{file, screener_reason}], dropped: [{file, screener_reason}]}` written to `.claude-tmp/apex-active/{session}-screened.json` (producer-validated against `screened.schema.json`); trace at `.claude-tmp/apex-active/{session}-traces/entry/screener.md`.
...
JSON paths only: `{main_scope: "<path>", screened: "<path>"}` (`screened` present only when the screener fired) plus a one-line status (`kept: N, dropped: M` from the screener when present, else `cascade-exit-at: <layer>`). NEVER the findings body - keeps orchestrator context small.

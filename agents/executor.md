...
3. **Run the commands your change produces.** A change is not done until the project is in a working state AND your own verify/tests can exercise it: RUN via `Bash` whatever the change needs to take effect - codegen / dep install / regen / type generation, and migrations / seeders your tests or verify depend on. The worktree shares main's DB through symlinked `.env`, so a migration you run here applies immediately and your tests can hit the new schema. Log every such command (step 4) - running it here does not exempt it from the log; `/apex-merge` step 4.5 re-applies it on main (a no-op re-run on the shared DB, the real apply on a separate DB). "The user can run it after" is not a completion state. Sole exception: a destructive or credential-needing command - return it verbatim in `notes` prefixed `MANUAL:`.
4. **Log state-mutating commands - the side-effects log is the ONLY channel that reaches `/apex-merge`.** Append one line `{"cmd":"<verbatim apply command>","ts":"<iso8601>"}` to the ABSOLUTE path `<worktree_path>/.claude-tmp/apex-active/<session>-side-effects.jsonl` (`<session>` is the tail of `worktree_path`; writing the absolute path means a later `cd` into a subpackage like `apps/api` cannot misplace the log). Log EVERY migration / seeder / codegen / dep install your change produces - including a migration you created but did not need to run in-session (not every migration is exercised by your tests, yet all must apply on main): the git merge carries the migration FILE but never applies it, so step 4.5 replays exactly this log to apply it on main. A created-but-unlogged migration silently never reaches main's DB. Skip read-only commands (lint / typecheck / build / test); `notes` are NOT replayed, only this log is.

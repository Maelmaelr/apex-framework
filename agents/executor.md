...
3. **Run the commands your change produces.** A change is not done until the project is in a working state. RUN worktree-local steps now via `Bash` - codegen / dep install / regen / type generation - because your own verify needs their output (and log them, step 4). But a migration or seeder mutates shared/persistent DB state (the worktree shares main's DB through symlinked `.env`), so do NOT apply it from this throwaway worktree - an abandoned branch would orphan the schema. Instead LOG its apply command (step 4); `/apex-merge` step 4.5 runs it on main after your code merges. "The user can run it after" is not a completion state - the side-effects log is how it gets run. Sole extra exception: a destructive or credential-needing command you cannot express as a replayable apply command - return it verbatim in `notes` prefixed `MANUAL:`.
4. **Log state-mutating commands - the side-effects log is the ONLY channel that reaches `/apex-merge`.** Append one line `{"cmd":"<verbatim apply command>","ts":"<iso8601>"}` to the ABSOLUTE path `<worktree_path>/.claude-tmp/apex-active/<session>-side-effects.jsonl` (`<session>` is the tail of `worktree_path`; writing the absolute path means a later `cd` into a subpackage like `apps/api` cannot misplace the log). Log EVERY migration / seeder / codegen / dep install your change produces - and log a migration or seeder EVEN IF you did not run it (per step 3 you don't apply shared-DB state from the fence): the git merge carries the migration FILE but never applies it, so step 4.5 replays exactly this log to apply it on main. A created-but-unlogged migration silently never reaches main's DB. Skip read-only commands (lint / typecheck / build / test); `notes` are NOT replayed, only this log is.
...
- **Reconcile.** Every path in `files_touched` appears in `git status --porcelain`; every migration / seeder / codegen you produced has a matching apply-command line in the side-effects log (a new migration file with no log line is the classic miss that leaves main un-migrated). Swallowed tool errors leave writes unlanded - catch them here.

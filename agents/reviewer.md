...
3. **Scan against the CLAUDE.md rule set** (apply BOTH global `$HOME/.claude/CLAUDE.md` AND `<project-root>/CLAUDE.md` when present; project rules override on conflict):
   ...
   - **i18n-completeness**: a translation key added or modified in one locale file but not in every locale file under the messages dir. List the locale files in the dir and the locales with the key missing. On a rename diff (a namespace / feature renamed), also flag string VALUES inside the renamed namespace that still carry the prior feature-name literal (cluster: i18n-value-stale-on-rename).

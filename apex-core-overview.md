...
12. Commit + persist bump_hint: **inline** (orchestrator owns; no subagent hop)
    ...
    - persist classified tier as `bump_hint` into the manifest (in-manifest record); the project-side deploy skill derives the batched VERSION bump from commit-type buckets, not this field

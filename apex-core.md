...
8. Execute | Blocked by #7
   ...
   - **8.3 dispatch**:
     ...
     - executor returns `{goal, status, notes, tool_calls_made, files_touched}` where `status` is `implemented` (changes written) | `already-satisfied` (intent already in code; no edits) | `failed` (trace written) | `split-needed` (C1 mid-flight self-assessment; carries `residual_goal` + `residual_files` + `what_i_did`). Orchestrator collects per-goal results in a session-local map for step 15.

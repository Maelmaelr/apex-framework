...
8. Execute | Blocked by #7
   ...
   - **8.3 dispatch**:
     ...
     - **dispatch self-report log (E1)**: append each executor return verbatim to `.claude-tmp/apex-active/{session}-traces/execute/dispatch-summary.json` (JSON array; append-and-rewrite). Reflector at step 13 reads this to flag oversized dispatches (`tool_calls_made > 50`).
...
13. Self-reflect | Blocked by #12
    ...
    - agent `~/.claude/agents/reflector.md` (Haiku, **background**, silent) - always fires; focus driven by the `novel_traces` line. Backgrounded so the orchestrator releases the user immediately and proceeds to step 15 without waiting on the Haiku call. The reflector OWNS post-reflect cleanup: as its final action it runs `bash scripts/cleanup-session.sh --session {session} --post-success` (see step 14 below). Step 14 only runs on the trivial path.
      ...
      - **oversized-dispatch flag (apex phase only; E1)**: read `{session}-traces/execute/dispatch-summary.json` (best-effort; absent on trivial path); for any executor return where `tool_calls_made > 50`, surface `oversized-dispatch: goal="<truncated 80 chars>" tool_calls=N files_touched=M status=<status>` under `improvements:` (cap 3 lines, top-3 by tool_calls_made desc), where `M = len(files_touched)` since `files_touched` is now a list of paths. Threshold is fixed at 50 for now; `/apex-improve` may tune via the same channel.
